...
const TopWear = () => {
  ...
  return (
    <div>
      <div>
      <div className='lg:mx-20 md:my-5 mx-1 sm:mx-10'>
      <div className='md:flex md:justify-between items-center p-4'>
  <h1 className='font-bold lg:text-4xl text-3xl md:py-0 py-3 nav'>Western Wear</h1>
  <button    onClick={()=>navigate("/western/wear/cloths")} className='hover:bg-red-900 hover:text-white text-lg rounded-xl border border-red-900 px-8 py-2 transition duration-300'>
    See more
      ...
      <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-1 md:gap-4'>
        {data.map((item, index) => (
            <div onClick={()=>navigate("/western/wear/cloths")} key={index} className='border cursor-pointer  border-gray-200 p-4'>
                <img src={item.img} alt="img" className='scaleImg '/>
                <div className='text-center pt-1'>
                {/* <div className='fle border'> */}
                <h3>{item.title}</h3>
                <strong className=' py-1 text-red-700'>₹{item.price}</strong><br />
